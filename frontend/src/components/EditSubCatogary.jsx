...
const EditSubCatogary = (props) => {
  ...
  const editSubCAtegory = async (e) => {
    ...
    const handleDeleteClick = (id) => {
        ...
        setData((prev) => ({
            ...prev,
            category: prev.category.filter((value) => value !== id)
        }));
    }
    console.log(data)
  ...
  return (
    <section className=" fixed w-full h-full top-0 right-0 flex justify-center items-center bg-neutral-500 opacity-90 ">
      <form
        onSubmit={(e) => editSubCAtegory(e)}
        className="w-[90%]   md:w-[70%] bg-gray-200 rounded-md shadow-xl p-6"
      >
           ...
           <div className="w-[90%] h-auto p-2 mt-4 ml-4 flex gap3">
            {
                data?.category?.map((value , index) => {
                    return <div key={index} className="bg-green-400 p-2 rounded-md text-[10px] ml-1       text-white flex items-center">{value} <RxCross2 size={18} onClick={() => handleDeleteClick(value)} /></div>
                })
            }
           </div>
        <select
          onChange={(e) => {
            ...
            console.log(selectedCatogary , "selected catogary")
            setData((prev) => {
                return {...prev,
                    category : [...prev.category , selectedCatogary]
                }
            })
          }}
...
export default EditSubCatogary;
